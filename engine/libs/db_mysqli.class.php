...
class PowerBBSQL
{
	...
	function SetInformation($host,$db_username,$db_password,$db_name,$db_type)
	{
		$this->host        = $host;
	...
	{
		$out = @mysqli_fetch_assoc($result,MYSQL_ASSOC);
		return $out;
		$this->sql_free_result($result);
	}
	...
	{
		$result = @mysqli_query($this->sql_connect(),"SELECT * FROM " . $table);

		if (!$result)
		{
			$err = @mysqli_errno($this->sql_connect());
