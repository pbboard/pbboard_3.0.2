	...
	function _AddAddonsStart()
	{
		...
		if (empty($PowerBB->_FILES['files']['name']))
		{
         ...
         // chek chmod  HooksCache.php
          ...
          if (!is_writable($url))
          {
		   $Permission = chmod('../cache/HooksCache.php', 0777);
		   if (!$Permission)
		   {
		    $PowerBB->functions->error_no_foot("Change Permissions File: cache/HooksCache.php To chmod 0777- يرجى اعطاء تصريح 777 لملف cache/HooksCache.php ليتم تركيب الاضافات");
		   }
		 }
					...
					if(!isset($plugin['plugin']['templates']['template'][1]))
					{
			       		foreach($StyleList as $Style)
						{
							...
							if($Templates['attributes']['type'] == 'new'){
								...
								$templates_dir = ("../cache_templates/".$Templattitle."_".$StyleId.".php");
								if (file_exists($templates_dir))
								{
							 	 $cache_del = unlink($templates_dir);
								}
...
';
       ...
       $x=1;
		...
		$data .= "\n\n".'//end of cache file'."\n";
		$data .= '?>';
		$open = fopen('../cache/HooksCache.php','w');
		if (!$open)
		{
		  $Permission = chmod('../cache/HooksCache.php', 0777);
		  if (!$Permission)
