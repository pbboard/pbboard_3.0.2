...
class PowerBBTopicMOD
{
	...
	function _ShowTopic()
	{
      ...
      $hookstopic = $PowerBB->functions->get_hooks("topic");
       ...
       $image_path = $PowerBB->_CONF['rows']['style']['image_path'];
		...
		($CheckOnline) ? $PowerBB->template->assign('status',"<img title='Online' alt='Online' src='$image_path/icon_user_online.gif' />") : $PowerBB->template->assign('status',"<img title='offline' alt='offline' src='$image_path/icon_user_offline.gif' />");
     ...
     $this->Info['user_info'] = $PowerBB->functions->CleanVariable($this->Info['user_info'],'html');
       ...
       $this->Info['reason_edit'] = $PowerBB->Powerparse->censor_words($this->Info['reason_edit']);
         ...
         //// get count perpage
		  ...
		  $PowerBB->template->assign('count',$perpage);
			if ($PowerBB->_GET['last_post'])
			{
				...
				$last_replyNumArr = $PowerBB->DB->sql_query("SELECT id FROM " . $PowerBB->table['reply'] . " WHERE subject_id='$subject_id' AND delete_topic<>1 AND review_reply<>1 ORDER BY id DESC LIMIT 0,1");
				$last_reply = $PowerBB->DB->sql_fetch_array($last_replyNumArr);
				if ($SubjectInfReplyNum > $PowerBB->_CONF['info_row']['perpage'])
				{
					$PowerBB->functions->redirect2('index.php?page=topic&amp;show=1&amp;id='.$PowerBB->_GET['id'].'&amp;count='.$perpage.'#'.$last_reply['id']);
					exit;
				}
				else
				{
					$PowerBB->functions->redirect2('index.php?page=topic&amp;show=1&amp;id='.$PowerBB->_GET['id'].'#'.$last_reply['id']);
					exit;
				}
			}
          }
		...
		while ($n > $this->x)
		{
	        ...
	        $hooks_topic_bady = $PowerBB->functions->get_hooks("topic_bady");
     ...
     if ($PowerBB->_CONF['info_row']['samesubject_show'])
	 {
       ...
       if($PowerBB->_CONF['group_info']['see_who_on_topic'])
       {
        ...
        // pager Duwn Subject
         ...
         $hooks_topic_end = $PowerBB->functions->get_hooks("topic_end");
