...
class PowerBBForumMOD
{
	...
	function _GetSubSection()
	{
		...
		{		      @include("cache/sectiongroup_cache".$this->Section['id'].".php");
	          $forums = unserialize(base64_decode($sectiongroup_cache));
       ...
       if ($section != is_numeric($section))
		{
		   ...
		   $PowerBB->functions->error($PowerBB->_CONF['template']['_CONF']['lang']['no_search_results']);
			...
			$SubjectSearchArr['pager']['location'] 	= 	'index.php?page=forum&amp;option=1&amp;section=' . $section . '&amp;keyword=' . $keyword;
