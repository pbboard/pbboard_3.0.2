	      ...
	      if ($PowerBB->_CONF['info_row']['show_online_list_today'] == 1)
	      {
			if ($PowerBB->_CONF['info_row']['mor_hours_online_today'] == '0')
			{
				if ($PowerBB->_CONF['date'] != $PowerBB->_CONF['info_row']['today_date_cache'])
				{
					...
					if ($PowerBB->_COOKIE[$PowerBB->_CONF['today_cookie']] != $PowerBB->_CONF['info_row']['today_date_cache'])
					{
		       ...
		       if ($page != 'login')
				{
				  if (empty($username))
		          {
					...
					if (isset($_SESSION['HTTP_USER_AGENT']))
					{
						...
						{
						 setcookie('PowerBB_username','');
						 setcookie('PowerBB_password','');
	...
	function _CheckClose()
	{
       ...
       //  Stop ipaddress banned
		...
		if ($IsBanned)
		{
			...
			$PowerBB->functions->error($PowerBB->_CONF['template']['_CONF']['lang']['ban_IP'].'<br />'.$PowerBB->_CONF['template']['_CONF']['lang']['reason_ban'].' '.$IsBanned['reason'],$stop,$stop);
		}
			...
			if ($value !='')
			{
			 ...
			 $PowerBB->_GET[$var_name] 	= 	$PowerBB->functions->CleanVariable($PowerBB->_GET[$var_name],'sql');
   			...
   			if ((@strstr(strtolower($sql_get), "select")) or
       			...
       			(@strstr($sql_get, "(")) or
       			(@strstr($sql_get, ")")) or
       			(@strtolower(strstr($sql_get), "onload")))
       		{
       		    exit('No direct script access allowed');
   			}
  		}
	...
	function _TemplateAssign()
	{
     ...
     if ($PowerBB->_CONF['info_row']['show_ads'] == 1)
        ...
        //////
	       ...
	       $PowerBB->_CONF['template']['while']['Messages'] = $PowerBB->core->GetList($CahtArr,'chat');
	       }
    ...
    // Get member alerts
     ...
     ////////
	  ...
	  $PowerBB->_CONF['template']['while']['SmlList'] = $PowerBB->icon->GetCachedSmiles();
       ...
       $PowerBB->_CONF['template']['_CONF']['lang']['copyright'] = str_replace("pbboard.com","pbboard.info",$PowerBB->_CONF['template']['_CONF']['lang']['copyright']);
       $PowerBB->_CONF['template']['_CONF']['lang']['copyright_archive'] = str_replace("pbboard.com","pbboard.info",$PowerBB->_CONF['template']['_CONF']['lang']['copyright_archive']);
      ...
      // Get register date
         ...
         $PowerBB->template->assign('ForumAdress',$PowerBB->functions->GetForumAdress());
          ...
          // Get On sidebar list pages  05-09-2015 v3.0.2
		...
		{
		$PowerBB->_CONF['template']['calendar_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'special_subject')
		{
		$PowerBB->_CONF['template']['special_subject_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'register')
		{
		$PowerBB->_CONF['template']['register_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'login')
		{
		$PowerBB->_CONF['template']['login_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'usercp')
		{
		$PowerBB->_CONF['template']['usercp_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'rules')
		{
		$PowerBB->_CONF['template']['rules_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'static')
		{
		$PowerBB->_CONF['template']['static_page'] = 'primary_oof';
		}
		if($PowerBB->_GET['page'] != 'member_list')
		{
		$PowerBB->_CONF['template']['member_list_page'] = 'primary_oof';
		}
       //
