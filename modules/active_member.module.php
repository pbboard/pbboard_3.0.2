...
class PowerBBCoreMOD
{
	...
	function _Index()
	{
		...
		$GroupInfo = $PowerBB->group->GetGroupInfo($GroupArr);
		...
		$RequesInfo = $PowerBB->core->Deleted($RequesArr,'requests');
			...
			@ob_start();
			setcookie("PowerBB_username",$ReqMemberInfo['username'],time()+31536000, NULL ,NULL, NULL, TRUE);
			setcookie("PowerBB_password",$ReqMemberInfo['password'],time()+31536000, NULL ,NULL, NULL, TRUE);
			@ob_end_flush();
