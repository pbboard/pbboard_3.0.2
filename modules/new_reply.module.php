                    ...
                    if (!$PowerBB->_CONF['group_info']['admincp_allow'])
                     {
			     		...
			     		{
		     			     	  $PowerBB->functions->ShowHeader($PowerBB->_CONF['template']['_CONF']['lang']['Mistake']);
		                    	  $PowerBB->functions->error($PowerBB->_CONF['template']['_CONF']['lang']['post_text_max']);
				     	 }
                 ...
                 else
				 {
                   ...
                   $code_hooks = $PowerBB->functions->get_hooks("insert_reply");
						...
						and !$PowerBB->functions->ModeratorCheck($this->SectionInfo['id']))
						{
						...
						}		     			else
		     			{
				     		...
				     		else
				     		{
					          ...
					          $reply_number_r = $PagerReplyNumArr-$roun_ss_r+1;
							  ...
							  $PowerBB->functions->header_redirect('index.php?page=topic&amp;show=1&amp;id=' . $this->SubjectInfo['id'] . '&amp;count=' . $perpage_r  . '#' . $PowerBB->reply->id);
	                         }
	                     }
		            }
		     		else
		     		{
						...
						{
							$PowerBB->_CONF['template']['ReplierInfo']['register_date'] = $PowerBB->functions->date($PowerBB->_CONF['template']['ReplierInfo']['register_date']);
						}
