				                               ...
				                               foreach ($subs_sub as $sub_sub)
												{
												   if ($sub['id'] == $sub_sub['parent'])
				                                    {
											            @include("cache/sectiongroup_cache".$sub_sub['id'].".php");
												        $groups = unserialize(base64_decode($sectiongroup_cache));
   ...
   function _StartSearchUsernameSubject()
	...
	function _StartSearchTag()
	{
       ...
       and $section != is_numeric($section))
		...
		if ($tag_nm > $PowerBB->_CONF['info_row']['subject_perpage'])
        {
		...
		$TagArr['pager']['location'] 	= 	'index.php?page=search&amp;option=5&amp;tag=' . $tag . '&amp;sort_order='.strtoupper($sort_order);
		$TagArr['pager']['var'] 		= 	'count';

		$TagArr['order'] 			= 	array();
		$TagArr['order']['field'] 	= 	'id';
		$TagArr['order']['type'] 	= 	strtoupper($sort_order);
		...
		{
			$stop = ($PowerBB->_CONF['info_row']['ajax_search'] and !$PowerBB->_POST['ajax']) ? false : true;
			$PowerBB->functions->error($PowerBB->_CONF['template']['_CONF']['lang']['taag_requested_does_not_exist'],$stop);
		}
